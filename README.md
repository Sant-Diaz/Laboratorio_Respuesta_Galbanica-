...
<p align="center">
  <img src="Imagen_7.jpeg" width="300">
</p>
...
# CONCLUSIONES 
- La señal GSR adquirida en la muñeca permitió identificar claramente un componente basal (tónico) y respuestas transitorias (fásicas). En particular, la fase inicial de estabilización (0–35 s) entregó una línea base consistente, sobre la cual se observaron incrementos y recuperaciones posteriores, lo que valida el protocolo de calibración para diferenciar cambios fisiológicos reales frente a variaciones pequeñas o ruido.
- La etapa de respiración (35–80 s) confirmó que es esencial calibrar el sistema de forma individual, porque la EDA/GSR presenta alta variabilidad entre personas (hidratación, temperatura de la piel, densidad/actividad sudomotora, presión de contacto y estado basal del sistema simpático). Por eso, usar una línea base propia y registrar el máximo y la recuperación durante la respiración permite definir umbrales personalizados de “estrés alto”, "estres moderado" y “estrés bajo” (idealmente como ΔV respecto a la base), logrando una clasificación más confiable que aplicar umbrales fijos iguales para todos. Además, se observó que cuando la señal alcanza valores cercanos al umbral de estrés alto, el retorno hacia niveles bajos puede tardar varios segundos o decenas de segundos: esto ocurre porque la respuesta de sudoración no se “apaga” de inmediato y la recuperación depende de la dinámica de la glándula y de la retirada progresiva del tono simpático, por lo que el descenso suele ser más lento que la subida.
- La adquisición en la muñeca demostró ser una opción práctica y consistente para monitoreo continuo, permitiendo registrar cambios de activación simpática y etiquetarlos en tiempo real mediante umbrales y un esquema de histéresis que evita saltos por pequeñas fluctuaciones. Además, el diseño eléctrico del sistema (resistencia de protección en serie y alimentación DC) mantuvo corrientes muy bajas, garantizando seguridad para el usuario según los criterios de IEC 60479, mientras se conservó la utilidad de la señal para análisis y seguimiento del estado fisiológico.
 # BIBLIOGRAFIA
